...
def test_first_text_delta_after_reasoning_content_is_not_dropped_sync():
    chunks = [
        _make_model_response_stream_chunk(
            Delta(content="", reasoning_content="thinking...")
        ),
        _make_model_response_stream_chunk(Delta(content="答案是2")),
        _make_model_response_stream_chunk(Delta(content=None), finish_reason="stop"),
    ]
    wrapper = AnthropicStreamWrapper(completion_stream=iter(chunks), model="claude-x")
    events = _drain_sync(wrapper)
    ...
    thinking_delta_indexes = [
        ...
        and event["delta"].get("thinking") == "thinking..."
    ]
    thinking_stop_indexes = [
        index
        for index, event in enumerate(events)
    ...
    text_start_indexes = [
        ...
        and event.get("index") == 2
        and event.get("content_block", {}).get("type") == "text"
    ]
    final_text_delta_indexes = [
        index
        for index, event in enumerate(events)
        if event.get("type") == "content_block_delta"
        and event["delta"].get("type") == "text_delta"
        and event["delta"].get("text") == "答案是2"
    ]
    empty_text_delta_indexes = [
        index
        for index, event in enumerate(events)
        if event.get("type") == "content_block_delta"
        and event["delta"].get("type") == "text_delta"
        and event["delta"].get("text") == ""
    ]

    assert thinking_delta_indexes
    assert empty_text_delta_indexes != final_text_delta_indexes
    assert thinking_stop_indexes
    assert text_start_indexes
    assert final_text_delta_indexes
    assert (
        thinking_delta_indexes[0]
        < thinking_stop_indexes[0]
        < text_start_indexes[0]
        < final_text_delta_indexes[0]
    )
